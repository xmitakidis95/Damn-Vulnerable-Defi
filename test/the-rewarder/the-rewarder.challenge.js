const { ethers, time } = require('hardhat'); // added time
...
describe('[Challenge] The rewarder', function () {
	...
	await ethers.provider.send("evm_increaseTime", [5 * 24 * 60 * 60]);// Start the next round of rewards   
	const RewarderExploit = await ethers.getContractFactory('RewarderExploit', attacker);
	this.attacker = await RewarderExploit.deploy();    
